...
import static com.or2go.core.Or2goConstValues.OR2GO_DELIVERY_STATUS_ASSIGN_ACCEPT;
import static com.or2go.core.Or2goConstValues.OR2GO_DELIVERY_STATUS_ASSIGN_REJECT;
import static com.or2go.core.Or2goConstValues.OR2GO_DELIVERY_STATUS_DELIVERY_DONE;
...
import com.or2go.or2gopartner.server.ActiveOrderCallback;
import com.or2go.or2gopartner.server.OrderStatusUpdateCallback;
...
public class OrderManager {
    ...
    boolean getPendingOrderList(ArrayList<Or2goOrderInfo> list, String mvendorid) {
        list.clear();
    ...
    public boolean getCompleteOrderList(ArrayList<Or2goOrderInfo> list) {
        ...
        for(int i=0;i<ordsz;i++)
        {
            ...
            if (ordinfo.oPayMode == OR2GO_PAY_MODE_COD || ordinfo.oPayMode == OR2GO_PAY_STATUS_NONE && outputFormat.format(date1) == String.valueOf(date.getDate())) {//paymode is not fixed.
                System.out.println("kkmlf" + ordinfo.getPayStatus());
                list.add(ordinfo);
            }
        }
        return true;
    ...
    public Integer getCompleteOrdersCount() {
        int ordsz = mCompletedOrderList.size();
        int listcnt=0;
        for(int i=0;i<ordsz;i++)
        {
    ...
    public Integer getProcessingingOrdersCount() {
        int ordsz = mOrderList.size();
        int listcnt=0;
        for(int i=0;i<ordsz;i++)
        {
    ...
    public Integer getReadyOrdersCount() {
        int ordsz = mOrderList.size();
        int listcnt=0;
        for(int i=0;i<ordsz;i++)
        {
    ...
    public Integer getOnDeliveryOrdersCount() {
        int ordsz = mOrderList.size();
        int listcnt=0;
        for(int i=0;i<ordsz;i++)
        {
    ...
    public Integer getDeliveredOrdersCount() {
    ...
    public synchronized boolean updateOrderStatus(String orderid, Integer sts, String desc)
    {
        ...
        else if (sts == OR2GO_DELIVERY_STATUS_DELIVERY_DONE){
    ...
    public void or2goGetPublicVendorList()
    {
        Message msg = new Message();
        msg.what = OR2GO_VENDOR_LIST_PUBLIC;	//fixed value for sending sales transaction to server
        msg.arg1 = 0;
        VendorListCallback sessioncb = new VendorListCallback(mContext);//Callback(mContext);
        sessioncb.setPublic();
        Bundle b = new Bundle();
        b.putParcelable("callback", sessioncb );
        msg.setData(b);
        gAppEnv.getCommMgr().postMessage(msg);
        gAppEnv.getVendorManager().setVendorListStatus(OR2GO_VENDOR_PRODUCTLIST_REQ);
    }
    ...
    public boolean rejectOrder(Or2goOrderInfo ordinfo, String pName, String rReject, String statusmsg)
    {
        ...
        try {
            ...
            pktHeader.accumulate("ordCust",ordinfo.oCustomer);
            pktHeader.accumulate("ordId", ordinfo.getId());
            pktHeader.accumulate("itemName", pName);
            pktHeader.accumulate("rejectReason", rReject);
