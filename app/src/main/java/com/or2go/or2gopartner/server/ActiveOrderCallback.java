...
public class ActiveOrderCallback extends CommApiCallback implements Parcelable {
    ...
    public Void call () {
        ...
        gAppEnv.getGposLogger().d("Active Order List API result is: " + result + "   server response=" + response);
        ...
        if (result > 0) {
            try {
                ...
                if (result.equals("ok")) {
                    ...
                    gAppEnv.getGposLogger().d("Active Order List API: adding active orders ");
                    ...
                    for (int i = 0; i < orderlist.length(); i++) {  // **line 2**
                        ...
                        String storeid = childJSONObject.getString("vendorid");
                        ...
                        for (int j = 0; j < orderDetails.length(); j++) {  // **line 2**
                                ...
                                OrderItem orditem = new OrderItem(itemid, itemname, Float.parseFloat(itemprice), Float.parseFloat(itemqnty),
                                    ...
                                    ProductManager productmgr = gAppEnv.getStoreManager().getProductManager();
                                    orditem.setProductInfo(productmgr.getProductInfo(itemid));
                                    ordinfo.addOrderItem(orditem);
                        }

                        gAppEnv.getGposLogger().d("Active Order List API: adding order #" + orderid);
